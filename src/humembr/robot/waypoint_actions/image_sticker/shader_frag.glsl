...
// When two images overlap, we blend them together. blendingPower controls how aggressive the
...
// cameras. blendingMinimum is more of a helper number to prevent numerical overflow.
...
float calculateScore(vec2 imageLocation) {
    vec2 locationNormalized = imageLocation * 2.0 - 1.0; // -1 to 1 (left to right, top to bottom)
    locationNormalized = 1.0 - abs(locationNormalized);  // small (near 0) on border, 1 in centers
