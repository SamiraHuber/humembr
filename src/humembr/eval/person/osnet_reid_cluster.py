...
from humembr.eval.person.reid_utils import get_osnet_reid_embeddings


class OsnetReidClusteringMethod(ClusteringMethod):
    ...
    def run(
        ...
        reid_embeddings_map = get_osnet_reid_embeddings(
            self.image_paths, osnet_extractor, model_name=model_name
        )

        if not reid_embeddings_map:
            print("No OSNet ReID embeddings extracted. Skipping ReID-only clustering.")
            return
